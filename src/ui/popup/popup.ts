...
async function init() {
  try {
    ...
    fontSelect.addEventListener('change', async () => {
      await manager.update({ font: { family: fontSelect.value } } as any);
      notifyContentScript('SETTINGS_CHANGED');
    });
    ...
    fontSize.addEventListener('change', async () => {
      await manager.update({ font: { size: parseInt(fontSize.value, 10) } } as any);
      notifyContentScript('SETTINGS_CHANGED');
    });
    ...
    for (const [key, displayName] of Object.entries(FEATURE_NAMES)) {
      ...
      const enabled =
        featureSettings && typeof featureSettings === 'object'
